...
function Visual({ type }: { type: string }) {
    ...
    <div className="flex gap-2 mt-4 items-end">
      {['#FAD1D8','#DBC0E7','#C9E6EE','#A8E6CF'].map((c, i) => (
        <motion.div key={i}
          ...
          style={{ width: 28+i*10, height: 28+i*10, background: c }}
          animate={{ y: [0,-6,0] }}
  ...
  if (type === 'grid') return (
    <div className="grid grid-cols-2 gap-1.5 mt-4 w-20">
      {['#C9E6EE','#FFD4B8','#FAD1D8','#B8C88D'].map((c,i) => (
        <motion.div key={i} className="w-8 h-8 rounded-xl" style={{ background:c }}
  ...
  if (type === 'waveform') return (
    <div className="flex items-end gap-1 mt-4 h-10">
      {[0.4,0.7,1,0.6,0.9,0.5,0.8,0.4,0.6,1,0.7,0.3].map((h,i) => (
        <motion.div key={i} className="w-1.5 rounded-full bg-[#7C7267]"
          style={{ height:`${h*100}%` }}
  ...
  if (type === 'lock') return (
    <motion.div className="text-5xl mt-4"
  ...
  if (type === 'call') return (
    <div className="flex gap-3 mt-4">
      {['🎬','📞'].map(e => (
        <motion.div key={e} className="w-12 h-12 rounded-2xl bg-white/60 flex items-center justify-center text-2xl shadow-sm"
          animate={{ scale:[1,1.07,1] }}
    ...
    <div className="flex items-center gap-2 mt-4 flex-wrap">
      {['Mar 30','Mar 29','Mar 28','Mar 27'].map((d,i) => (
        <div key={i} className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-[#433D35] opacity-50" />
          <span className="text-[11px] font-medium text-[#433D35]">{d}</span>
          {i < 3 && <div className="w-5 h-px bg-[rgba(67,61,53,0.2)]" />}
        </div>
      ))}
    </div>
  )
  ...
  if (type === 'dots') return (
    <div className="relative w-24 h-16 mt-4">
      {[{s:20,c:'#FAD1D8',t:'0%',l:'10%'},{s:14,c:'#DBC0E7',t:'20%',l:'40%'},{s:24,c:'#C9E6EE',t:'40%',l:'5%'},{s:16,c:'#A8E6CF',t:'10%',l:'65%'},{s:12,c:'#FFD4B8',t:'55%',l:'55%'}].map((b,i) => (
  ...
  return (
    <section id="features" ref={ref} className="relative py-28 px-6 bg-[#F7F4E9]">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Bento grid — cells fall in from different directions */}
