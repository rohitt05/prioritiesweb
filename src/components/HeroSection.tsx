...
export default function HeroSection() {
  ...
  const sc = useTransform(scrollYProgress, [0, 1], [1, 0.93])
    ...
    <section ref={ref} className="relative min-h-[100svh] flex flex-col items-center justify-center px-5 sm:px-8 pt-24 sm:pt-28 pb-28 sm:pb-36 overflow-hidden">
      ...
      <motion.div style={{ y: sy, opacity: op, scale: sc }} className="relative z-10 text-center max-w-4xl mx-auto w-full">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 mb-8 sm:mb-10 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-white border border-[rgba(67,61,53,0.1)] shadow-sm"
        >
          <span className="w-1.5 h-1.5 rounded-full bg-[#D4A373] animate-pulse" />
          <span className="text-[11px] sm:text-[12px] font-medium text-[#7C7267] tracking-wide">Now accepting early access</span>
        </motion.div>

        {/* Giant headline */}
        <h1 className="font-serif text-[clamp(48px,11vw,130px)] font-bold leading-[0.88] tracking-tight text-[#2C2720] mb-6 sm:mb-8">
          {WORDS.map((word, wi) => (
            <motion.span
              key={wi}
              ...
              initial={{ opacity: 0, y: 80, rotateX: -45, filter: 'blur(4px)' }}
              animate={{ opacity: 1, y: 0, rotateX: 0, filter: 'blur(0px)' }}
              transition={{ delay: 0.45 + wi * 0.12, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
              style={{ transformStyle: 'preserve-3d', perspective: '600px', display: 'inline-block' }}
            >
              {word}
            </motion.span>
          ))}
        ...
        <motion.p
          ...
          className="text-[15px] sm:text-[17px] md:text-[19px] text-[#7C7267] max-w-[400px] sm:max-w-[480px] mx-auto leading-relaxed mb-10 sm:mb-12"
        >
        ...
        >
          ...
          <motion.a href="#films" className="btn-outline text-[13px] sm:text-[14px] justify-center" whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}>
            See the App
          </motion.a>
        </motion.div>
      </motion.div>

      {/* Floating UI chips — hidden on small screens */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {[
          { label: 'Films of my day', sub: 'Monday · Mar 30', top: '20%', left: '2%',  rotate: -4, delay: 0 },
          { label: '📌 Priority #1',  sub: 'Active · Pinned', top: '15%', right: '2%', rotate: 5,  delay: 0.2 },
          ...
          <motion.div
            key={i}
            className="absolute bg-white border border-[rgba(67,61,53,0.09)] rounded-2xl px-4 py-3 shadow-sm min-w-[130px]"
            style={{ top: chip.top, bottom: chip.bottom, left: chip.left, right: chip.right, rotate: chip.rotate }}
            initial={{ opacity: 0, scale: 0.8, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ delay: 2.2 + chip.delay, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
          >
            <motion.div
              animate={{ y: [0, -8, 0] }}
              transition={{ delay: chip.delay, duration: 4 + i, repeat: Infinity, ease: 'easeInOut' }}
            >
              <div className="text-[11px] sm:text-[12px] font-semibold text-[#433D35]">{chip.label}</div>
      ...
      >
        ...
        <motion.div
          className="w-px h-8 sm:h-10 bg-gradient-to-b from-[#A89F8D] to-transparent"
